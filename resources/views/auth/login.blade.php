...
@section('content')
<div class="w-full max-w-5xl bg-white rounded-[32px] shadow-[0_25px_60px_-15px_rgba(99,102,241,0.18)] p-4 sm:p-6 md:p-8 flex flex-col lg:flex-row gap-6 lg:gap-10 border border-slate-100/90 my-auto relative z-10">

    {{-- LEFT PANEL: Vivid Animated Mesh Hero Graphic --}}
    <div class="w-full lg:w-1/2 rounded-[24px] hero-mesh p-8 md:p-10 flex flex-col justify-between text-white relative min-h-[300px] lg:min-h-[500px] overflow-hidden shadow-lg order-2 lg:order-1">
        {{-- Top Star Logo Icon --}}
        <div class="z-10">
            <div class="w-12 h-12 rounded-2xl bg-white/20 backdrop-blur-md border border-white/30 flex items-center justify-center shadow-md">
        ...
        <div class="space-y-3 z-10">
            ...
            <h2 class="text-xl md:text-2xl lg:text-3xl font-black text-white tracking-tight leading-snug">
    ...
    <div class="w-full lg:w-1/2 p-4 sm:p-6 md:p-8 flex flex-col justify-center order-1 lg:order-2">
        ...
        <p class="text-sm text-slate-500 font-medium mt-2 mb-8 leading-relaxed">
            Pilih peran Anda dan masukkan data login untuk mengakses sistem SIPBAR.
        </p>

        {{-- Session / Error Alert --}}
        @if ($errors->any())
            <div class="mb-6 p-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-700 text-sm space-y-2">
                ...
                <ul class="list-disc list-inside pl-1 text-xs text-rose-600 space-y-1">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            {{-- Role Select --}}
            <div>
                <label for="role" class="block text-sm font-extrabold text-slate-800 uppercase tracking-wider mb-2">
                    Peran Pengguna
                </label>
                <div class="relative">
            ...
            {{-- Identifier Input --}}
            <div>
                <label for="identifier" id="identifier-label" class="block text-sm font-extrabold text-slate-800 uppercase tracking-wider mb-2">
                    Email / NIP / NIS
                </label>
                <input type="text" id="identifier" name="identifier" value="{{ old('identifier') }}" required autofocus
                       ...
                       class="w-full px-4 py-3.5 bg-slate-50 border border-slate-200/90 rounded-xl text-sm font-semibold text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all shadow-sm" />
            </div>

            {{-- Password Input --}}
            <div>
                <label for="password" class="block text-sm font-extrabold text-slate-800 uppercase tracking-wider mb-2">
                    Password
                </label>
                <div class="relative">
                    <input type="password" id="password" name="password" required placeholder="••••••••••••"
                           class="w-full pl-4 pr-12 py-3.5 bg-slate-50 border border-slate-200/90 rounded-xl text-sm font-semibold text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all shadow-sm" />
            ...
            {{-- Submit Button --}}
            <button type="submit" class="w-full py-4 px-6 rounded-xl bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-extrabold text-sm uppercase tracking-wider shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/35 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2">
        ...
        <div class="relative my-6 text-center">
            ...
            <span class="relative bg-white px-4 text-xs font-extrabold text-slate-400 uppercase tracking-widest">SMKN 1 BANGSRI</span>
        </div>

        {{-- Footer Link --}}
        <div class="text-center">
            <a href="{{ route('home') }}" class="text-sm font-bold text-indigo-600 hover:text-indigo-800 transition-colors inline-flex items-center gap-2">
