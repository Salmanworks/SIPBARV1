<x-layouts.admin title="Edit Data Guru — {{ $guru->nama_lengkap }}">
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-soft">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight font-display">Edit Data Guru</h1>
                    <p class="text-xs text-slate-500 mt-0.5">Perbarui informasi guru <strong>{{ $guru->nama_lengkap }}</strong> beserta akunnya</p>
                </div>
            </div>
        </div>
        ...
        {{-- Form Container --}}
        <form method="POST" action="{{ route('admin.guru.update', $guru) }}" enctype="multipart/form-data" class="bg-white rounded-3xl border border-slate-200/80 shadow-soft p-6 md:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                ...
                <div class="grid gap-5 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="tel" id="no_hp" name="no_hp" value="{{ old('no_hp', $guru->no_hp) }}"
                ...
                <div>
                    <label for="foto" class="block text-xs font-extrabold uppercase tracking-wider text-slate-700 mb-2">
                        Foto Guru <span class="text-slate-400 font-normal lowercase">(opsional)</span>
                    </label>
                    @if($guru->foto)
                        <div class="mb-3 flex items-center gap-3">
                            <img src="{{ asset('storage/'.$guru->foto) }}" alt="Foto Guru" class="w-16 h-16 rounded-2xl object-cover border-2 border-slate-200">
