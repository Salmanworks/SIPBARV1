<x-layouts.admin title="Tambah Siswa Baru">
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-soft">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight font-display">Tambah Siswa Baru</h1>
                    <p class="text-xs text-slate-500 mt-0.5">Daftarkan siswa beserta akun login otomatis (password default = NIS)</p>
                </div>
            </div>
        </div>
        ...
        {{-- Form Container --}}
        <form method="POST" action="{{ route('admin.siswa.store') }}" enctype="multipart/form-data" class="bg-white rounded-3xl border border-slate-200/80 shadow-soft p-6 md:p-8 space-y-6">
            @csrf
            ...
            <div class="space-y-5">
                <div class="grid gap-5 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="text" id="nis" name="nis" value="{{ old('nis') }}" placeholder="Contoh: 202401001" required
                    ...
                    <div>
                        ...
                        <input type="text" id="nama_lengkap" name="nama_lengkap" value="{{ old('nama_lengkap') }}" placeholder="Nama lengkap siswa..." required
                ...
                <div class="grid gap-5 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="text" id="kelas" name="kelas" value="{{ old('kelas') }}" placeholder="Contoh: X-IPA-1"
                    ...
                    <div>
                        ...
                        <select id="jurusan" name="jurusan"
                            ...
                            @foreach($jurusans as $jrs)
                                <option value="{{ $jrs }}" @selected(old('jurusan') === $jrs)>{{ $jrs }}</option>
                            @endforeach
                        </select>
                        @error('jurusan')
                            <p class="text-xs font-semibold text-rose-500 mt-1.5">{{ $message }}</p>
                ...
                <div class="grid gap-5 sm:grid-cols-2">
                    <div>
                        <label for="email" class="block text-xs font-extrabold uppercase tracking-wider text-slate-700 mb-2">
                            Alamat Email <span class="text-slate-400 font-normal lowercase">(opsional)</span>
                        </label>
                        <input type="email" id="email" name="email" value="{{ old('email') }}" placeholder="siswa@sekolah.sch.id"
                ...
                <div class="grid gap-5 sm:grid-cols-2">
                    <div>
                        <label for="password" class="block text-xs font-extrabold uppercase tracking-wider text-slate-700 mb-2">
                            Password <span class="text-slate-400 font-normal lowercase">(default: NIS)</span>
                        </label>
                ...
                <div>
                    <label for="foto" class="block text-xs font-extrabold uppercase tracking-wider text-slate-700 mb-2">
                        Foto Siswa <span class="text-slate-400 font-normal lowercase">(opsional, max 2MB)</span>
                    </label>
