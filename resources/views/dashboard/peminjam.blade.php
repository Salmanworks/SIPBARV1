<x-layouts.admin title="Dashboard Peminjam">
    <div class="space-y-7">
        ...
        <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-blue-500 via-indigo-500 to-violet-600 p-8 md:p-10 text-white shadow-xl shadow-blue-500/25">
            ...
            <div class="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-5">
                ...
                <a href="{{ route('peminjam.pengajuan.create') }}" class="flex-shrink-0 inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-2xl bg-white text-blue-700 font-extrabold text-xs uppercase tracking-wider shadow-lg hover:bg-blue-50 hover:scale-105 transition-all">
        ...
        <div class="bg-white rounded-3xl border border-slate-200/70 shadow-sm overflow-hidden">
            <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                ...
                <a href="{{ route('peminjam.riwayat.index') }}" class="text-xs font-bold text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-xl transition-all">Lihat Semua</a>
            </div>
            ...
            @if($recent->isEmpty())
                <div class="p-14 text-center">
                    ...
                    <a href="{{ route('peminjam.pengajuan.create') }}" class="inline-flex items-center gap-2 mt-4 px-5 py-2.5 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold text-xs shadow-md hover:scale-105 transition-all">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                        Ajukan Sekarang
                    </a>
                </div>
                    ...
                    @foreach($recent as $item)
                        <div class="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-blue-50/30 transition-colors group">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <a href="{{ route('peminjam.riwayat.show', $item) }}" class="px-3 py-1.5 bg-slate-100 hover:bg-blue-100 hover:text-blue-700 text-slate-700 font-bold text-xs rounded-xl transition-all">Detail</a>
                            </div>
                        </div>
                    @endforeach
                </div>
