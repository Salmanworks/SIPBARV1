<x-layouts.admin title="Edit Barang — {{ $barang->nama_barang }}">
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-soft">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight font-display">Edit Barang</h1>
                    <p class="text-xs text-slate-500 mt-0.5">Perbarui rincian informasi dan status inventaris untuk <strong>{{ $barang->nama_barang }}</strong></p>
                </div>
            </div>
        ...
        {{-- Form Container --}}
        <form method="POST" action="{{ route('admin.barang.update', $barang) }}" enctype="multipart/form-data" class="bg-white rounded-3xl border border-slate-200/80 shadow-soft p-6 md:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-6 lg:grid-cols-3">
                ...
                <div class="lg:col-span-2 space-y-5">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="kode_barang" name="kode_barang" value="{{ old('kode_barang', $barang->kode_barang) }}" required
                                       class="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all uppercase @error('kode_barang') border-rose-500 bg-rose-50/30 @enderror" />
                            </div>
                            @error('kode_barang')
                    ...
                    <div class="grid gap-5 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="stok" name="stok" min="0" value="{{ old('stok', $barang->stok) }}" required
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <label class="relative flex items-center gap-3 p-3.5 rounded-2xl border border-slate-200 bg-slate-50 cursor-pointer hover:bg-emerald-50/60 hover:border-emerald-200 transition-all has-[:checked]:bg-emerald-50 has-[:checked]:border-emerald-500 has-[:checked]:ring-2 has-[:checked]:ring-emerald-500/20">
                                <input type="radio" name="kondisi" value="baik" class="w-4 h-4 text-emerald-600 border-slate-300 focus:ring-emerald-500" @checked(old('kondisi', $barang->kondisi->value) === 'baik') required />
                                <div>
                            ...
                            <label class="relative flex items-center gap-3 p-3.5 rounded-2xl border border-slate-200 bg-slate-50 cursor-pointer hover:bg-rose-50/60 hover:border-rose-200 transition-all has-[:checked]:bg-rose-50 has-[:checked]:border-rose-500 has-[:checked]:ring-2 has-[:checked]:ring-rose-500/20">
                                <input type="radio" name="kondisi" value="rusak" class="w-4 h-4 text-rose-600 border-slate-300 focus:ring-rose-500" @checked(old('kondisi', $barang->kondisi->value) === 'rusak') />
                                <div>
                    ...
                    <div>
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="4" placeholder="Tuliskan spesifikasi lengkap, nomor seri, atau keterangan barang..."
                                  class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all resize-none @error('deskripsi') border-rose-500 bg-rose-50/30 @enderror">{{ old('deskripsi', $barang->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="text-xs font-semibold text-rose-500 mt-1.5">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="bg-slate-50/80 border-2 border-dashed border-slate-300 rounded-3xl p-5 text-center flex flex-col items-center justify-center min-h-[280px] relative group hover:border-blue-500 hover:bg-blue-50/30 transition-all">
                        ...
                        {{-- Preview Wrapper --}}
                        <div id="preview-wrapper" class="w-full h-full flex flex-col items-center">
                            <img id="preview-img" src="{{ $barang->fotoUrl() }}" alt="{{ $barang->nama_barang }}" class="w-full h-48 object-cover rounded-2xl shadow-sm border border-slate-200 mb-3" />
                            <p class="text-[11px] font-bold text-slate-600 truncate max-w-[200px]" id="preview-filename">
                                {{ $barang->foto ? basename($barang->foto) : 'Foto Default' }}
                            </p>
                            <span class="text-[10px] text-blue-600 font-bold mt-1">Klik untuk mengganti foto barang</span>
                        </div>
                    </div>
                    @error('foto')
                        <p class="text-xs font-semibold text-rose-500 mt-1.5">{{ $message }}</p>
