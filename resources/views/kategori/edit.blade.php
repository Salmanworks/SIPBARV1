<x-layouts.admin title="Edit Kategori — {{ $kategori->nama_kategori }}">
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 md:p-8 rounded-3xl border border-slate-200/80 shadow-soft">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight font-display">Edit Kategori</h1>
                    <p class="text-xs text-slate-500 mt-0.5">Perbarui nama atau keterangan kategori <strong>{{ $kategori->nama_kategori }}</strong></p>
                </div>
            </div>
        </div>
        ...
        {{-- Form Container --}}
        <form method="POST" action="{{ route('admin.kategori.update', $kategori) }}" class="bg-white rounded-3xl border border-slate-200/80 shadow-soft p-6 md:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="4" placeholder="Tuliskan keterangan mengenai pengelompokan jenis barang ini..."
                              class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all resize-none @error('deskripsi') border-rose-500 bg-rose-50/30 @enderror">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
                    @error('deskripsi')
